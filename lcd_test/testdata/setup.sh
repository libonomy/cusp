...
sleep 1s
echo ${PASSWORD} | ./build/cuspcli tx gov submit-proposal --home ${HOME} --from ${ADDR} --chain-id ${CHAIN} --type text --title test --description test_description --deposit 10000flby --yes
sleep 1s
echo ${PASSWORD} | ./build/cuspcli tx gov deposit --home ${HOME} --from ${ADDR} --chain-id ${CHAIN} ${PROPOSALID} 1000000000flby --yes
sleep 1s
...
sleep 1s
echo ${PASSWORD} | ./build/cuspcli tx staking unbond --home ${HOME} --from ${ADDR} ${VALIDATOR} 100flby --yes --chain-id ${CHAIN}
